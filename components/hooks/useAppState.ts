...
import { useGuestOrderPoller }  from "./useGuestOrderPoller";
...
export function useAppState() {
  ...
  // ── Guest Order Poller (QR menyu buyurtmalari) ────────────────
  useGuestOrderPoller({
    enabled: true,
    onNewOrders: (newOrders) => {
      setOrders((prev) => {
        ...
        fresh.forEach((o) => {
          const tableLabel = o.tableNumber ? `Stol #${o.tableNumber}` : "Olib ketish";
          notif.push(
            `📱 Yangi QR buyurtma — ${tableLabel}`,
            `${o.stallName} · ${o.items.length} ta mahsulot · ${formatNumber(o.totalAmount)} so'm`,
            "info",
            { actionLabel: "Buyurtmani ko'rish", actionTab: "orders" }
          );
          setWsEvents((wsOld) => [{
            id:   getUniqueId("ws"),
            time: getSystemTime(),
            text: `📱 QR buyurtma: ${o.stallName} ${tableLabel} — ${formatNumber(o.totalAmount)} so'm`,
  ...
  // ref so guest order poller can append without stale closure
  // (setWsEvents stable reference suffices — React guarantees this)
